{
  # Machines
  azazel = [
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIJBnYnuUen7HpY43vvqgWzF7ZQ1UFkHCDzvSqGghwV0G"
  ];

  theo = [
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIGG1QXBggbiauDamPAWS7L0CjtHnr3kTnuK1oNBRoiPD"
  ];

  # Users
  jtojnar = [
    "ssh-rsa  jtojnar@theo"
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIJPoWhe9QjOBT2OzkMQ4UVLM/vV5d6jE/Aqswn6Ixc+E tojnar@ttc.cz"
  ];

  jtojnar-redmi = [
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAICdzt7PW7zi1uZIMdtPUux4cY0gG4CmyR4PZuqEM+wtV jtojnar@redmi"
  ];

  otec = [
    "ssh-rsa  Otec@HP"
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIFw68VIc52JPJZinaQE04g2CgthRe7HJ7E8TGc8cJb8h tojnar+fotrbook@gmail.com"
  ];
}

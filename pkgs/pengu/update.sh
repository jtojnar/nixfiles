...
env NIX_PATH="nixpkgs=channel:nixos-unstable" nix-prefetch-github jtojnar pengu > $srcDir/src.json
platform=$(nix-instantiate --eval --json -E 'builtins.currentSystem')
src=$(nix-build --no-out-link -A "outputs.packages.${platform}.pengu.src")

...
currentVersion=$(nix-instantiate --eval --json --expr 'with import ./. {}; flarum.version' | jq --raw-output '.')
...
update-source-version flarum "$latestVersion"
...
sourceDir="$(nix-build -A flarum.src --no-out-link)"
tempDir="$(mktemp -d)"
